...
public abstract class XMLNodeWithChildren extends WrappedXMLNode {
    ...
    public List<XMLNode> getChildNodes() {
        ...
        for (int i = 0; i < children.getLength(); i++) {
            ...
            switch (item.getNodeType()) {
                case Node.ELEMENT_NODE -> childNodes.add(new XMLNodeElement((Element) item));
                case Node.TEXT_NODE -> {
                    if (!item.getNodeValue().isEmpty()) {
                        childNodes.add(new XMLNodeText((Text) item));
                    }
                }
                case Node.COMMENT_NODE -> childNodes.add(new XMLNodeComment((Comment) item));
            }
        }
        return childNodes;
    }
    ...
    public List<String> getStringList(String name) {
        ...
        for (XMLNode childNode : getChildNodes()) {
            if (childNode instanceof XMLNodeElement element) {
    ...
    public void set(String name, Object value) {
        ...
        for (XMLNode childNode : getChildNodes()) { // 遍历已有的子节点
            if (!(childNode instanceof XMLNodeWithChildren node)) { // 不是可以操作的元素
                continue;
            }
    ...
    public void set(Object value) {
        for (XMLNode childNode : getChildNodes()) {
            if (childNode instanceof XMLNodeText text) {
